...
SAFE_ENTITY_CHARS = frozenset('abcdefghijklmnopqrstuvwxyz'
...
def is_safe_entity_name(entity: Text) -> bool:
  return (entity not in BAD_ENTITY_NAMES
          and not frozenset(entity) - SAFE_ENTITY_CHARS)
...
def safe_entity_name(entity: Text) -> Text:
  ...
  if entity == '':
    ...
    return entity
  return ''.join(
      c if c in SAFE_ENTITY_CHARS else '{:02x}'.format(ord(c)) for c in entity)
